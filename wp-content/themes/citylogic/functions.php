 ...
 * @package CityLogic
 */
define( 'CITYLOGIC_THEME_VERSION' , '1.0.69' );
...
function citylogic_admin_notice() {
	...
	if ( !empty( $message['text'] ) && !get_user_meta( $user_id, 'citylogic_admin_notice_' .$message['id']. '_dismissed' ) ) {
		$class = 'notice notice-success is-dismissible';
		printf( '<div class="%1$s"><p>%2$s</p><p><a href="?citylogic-admin-notice-dismissed&citylogic-admin-notice-id=%3$s">Dismiss this notice</a></p></div>', esc_attr( $class ), $message['text'], $message['id'] );
	}
}
